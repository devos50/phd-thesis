...
Even though blockchain technology provides the means to maintain a distributed ledger without trusted authorities, open enrolment is not required for many industrial use cases, or is even undesirable.
...
There currently is a proliferation of different types of assets, fragmented across many blockchain implementations~\cite{borkowski2018towards}.
...
\begin{enumerate}
	...
	\item We improve TrustChain, a tamper-proof, distributed log used by \ModelName{}. Our improvements enable concurrent transactions and increase scalability (Section~\ref{sec:blockchain_accounting}).
...
There is little research on how to achieve interoperability between permissioned blockchains, even though this is also a concern in private environments.
...
This window enables price speculation by the swap initiator by keeping the assets of the other party locked until the asset price goes in the favour of the initiator.
...
Even though this design would avoid the need for a shared log, it would result in the fragmentation of trade records across potentially many ledgers, making it infeasible to accurately determine in which trades a specific trader is currently involved.
...
We refer to the amount of risked assets as the \MarginalStake{}.
...
\subsubsection{Additional Comments on Risk Mitigation}
...
\ModelName{} users participate in a peer-to-peer network, which is used to send point-to-point messages to other users.
...
If $ M $ agrees with the proposal and also wishes to trade with $ T $, $ M $ constructs an \texttt{Agreement} record, which includes the identifier of the order being fulfilled and the proposal created by the taker (including the taker's signature).
...
Nonetheless, we note that \ModelName{} does not introduce a theoretical bound on the loss of a risktaker over time, but delegates the risk management to the risktaker by assuming a trust mechanism to determine the trust threshold.
...
This yields superior scalability compared to other ledgers but allows for the situation where some malicious activity targeted at the ledger might go undetected for some time, for example, hiding specific transactions.
...
\begin{figure*}[t]
	...
	\begin{subfigure}[t]{.45\textwidth}
		...
		\label{fig:xchange_trustchain_tutorial_1}
	\end{subfigure}%
	\begin{subfigure}[t]{.45\textwidth}
		\centering
		\captionsetup{width=.9\linewidth}
		...
		\label{fig:xchange_trustchain_tutorial_2}
	\end{subfigure}
	\par\bigskip
	\begin{subfigure}[t]{.45\textwidth}
		\centering
...
When users create more records with others, it leads to the directed acyclic graph (DAG) structure, as shown in Figure \ref{fig:xchange_trustchain}.
Figure \ref{fig:xchange_trustchain} shows seven blocks, created by seven unique users.
...
Our solution is visualized in Figure \ref{fig:xchange_trustchain_tutorial_4}.
...
Figure~\ref{fig:trustchain_market} shows an \texttt{Order} record, created by maker $ A $.
...
Figure~\ref{fig:fraud_experiment_gained} clearly shows the effectiveness of the \emph{bounded obligations} strategy.
...
Even with this timing issue present, Figure~\ref{fig:fraud_experiment_gained} still shows that our risk mitigation is highly effective and is capable of significantly reducing fraud gains.
...
Finally, we highlight two promising research directions for further work.
