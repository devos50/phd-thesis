...
linetypes <- c("none" = "solid", "RESTRICT" = "longdash", "INC_SET(2)" = "dashed", "RESTRICT+INC_SET(2)" = "twodash")
dotstyles <- c("none" = 15, "RESTRICT" = 16, "INC_SET(2)" = 17, "RESTRICT+INC_SET(2)" = 18)
order <- c("none","RESTRICT","INC_SET(2)", "RESTRICT+INC_SET(2)")

dat <- read.csv("scalability.csv")
#dat$avg_bw <- dat$bw / dat$network_size / 1024.0
#dat$avg_trades <- dat$trades / 30.0
dat$system_load <- dat$instances
