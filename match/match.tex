\chapter{MATCH: A Decentralized Middleware for Fair Order Matchmaking}
\label{chapter:match}
...
Online commerce platforms are pervasively used in both traditional and emerging markets. %As a result, however, they increasingly become critical infrastructure in our society~\cite{oecd2019}\cite{azevedo2016} and, with raising influence of a few global players, a systemic risk to everyday consumers.
...
However, increasingly becoming critical infrastructure in our society, these platforms also bring systemic risks to everyday consumers~\cite{azevedo2016}\cite{oecd2019}.
...
The full potential of these developments is hard to assess yet, but some key trends are already evident such as an unprecedented concentration of power by a few centralized intermediaries~\cite{pepper2019}.
...
%Ineffective matchmaking by market operators not only decreases overall market efficiency and customer satisfaction, but also can result in price discrimination and unfair markets~\cite{Wu2015TheM}\cite{mikians2012}. #This sentence is redundant
...
Recently, however, the necessity of a trusted centralized market operator has been challenged by the advancements in decentralized technologies such as blockchain.
...
We argue, however, that these contradictions do not present an insurmountable obstacle for the implementation of efficient and fair matchmaking solutions. 
...
\textbf{Contributions.}
We present \emph{MATCH}, a decentralized middleware for fair order matchmaking, based on the principle of strictly decoupling the matching process from the negotiation of trade agreements. 
...
\section{Centralized Order Matchmaking}
\label{sec:centralized_order_matchmaking}
We introduce the required components for our system by first considering a centralized matchmaking architecture.
In this architecture, a single server stores and matches incoming orders.

\subsection{A Generic Model for Centralized Order Matchmaking}
A generic model for centralized order matchmaking is visualized in Figure~\ref{fig:central_exchange_architecture}.
%We first explain a centralized approach to order matching and elaborate how a matchmaker processes orders, see Figure~\ref{fig:central_exchange_architecture}.
Users create orders, which they then submit to the matchmaking server using a public API.
Many electronic markets deploy one or more gateways that filter out invalid orders and mitigate targeted attacks on the server such as a DDoS attack~\cite{mavroudis2019libra}.
Valid orders are inserted in the \emph{order book}, which is a local data structure that bundles all active and valid orders.
%The implementation of the order book is often optimized to process orders with a specific format as quickly as possible.
To prevent orders from being stored in the order book for an indefinite amount of time, each order usually times out after a specific duration.
The order is then removed from the order book.

Upon the insertion of a new order in the order book, the order is forwarded to the \emph{matching engine}.
...
For example, the price-time matching strategy is a predominant matching policy in financial trading systems~\cite{mavroudis2019libra}.
This particular policy first matches orders based on their price and then based on their creation time as a tie-breaker (prioritizing older orders).
The matching engine can establish multiple matches for a single order.
For example, a buy order for a large quantity of stocks in a stock market can be matched with multiple (smaller) sell orders.
When the matching engine has found one or more matches for an order, the matches should be \enquote{executed}, which is an application-dependent operation.
In a financial exchange, for example, the assets as specified in the matched orders should be exchanged between the order creators (this operation is also called settlement).
We model the component that executes established matches as the \emph{trading mechanism} (TM), which we consider external to the matchmaking architecture in Figure~\ref{fig:central_exchange_architecture}.
After established matches have been executed by the TM, the affected orders in the order book are updated (or removed if they are completed), and the order creators are notified of the execution.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=.8\linewidth]{match/assets/central_exchange_architecture}
	\caption{A generic model for centralized order matchmaking, using a single server.}
	\label{fig:central_exchange_architecture}
\end{figure}

\subsection{Towards Fair Order Matchmaking}
\label{sec:towards_fair_reliable_matchmaking}
The model in Figure~\ref{fig:central_exchange_architecture} allows the server operator to conduct unfair matchmaking, that is, manipulate the matching engine or policy to prioritize the orders of specific users.
In this work, we design a protocol (Section~\ref{sec:protocol}) and middleware (Section~\ref{sec:architecture}) that address this fairness issue.
A key design principle of our solution is to avoid reliance on any trusted market operator for order matchmaking.
%In particular, we propose a decentralized mechanism where any user can act as a matchmaker for others.
%\subsection{Towards Decentralized Order Matchmaking}
%\label{sec:towards_decentralized}
%Given the shift towards decentralized approaches without trusted intermediaries, we devise a decentralized order matchmaking architecture, not operated by a trusted intermediary and suitable for an Internet deployment.

As a first step towards decentralized matchmaking, we replicate the matchmaking server in Figure~\ref{fig:central_exchange_architecture} $ n $  times (where $ n > 1 $).
In this replicated matchmaking model, a user submits a new order to all $ n $ matchmaking servers, and all matchmaking servers forward established matches to the same TM.
If the market operator did manipulate a strict subset of matchmaking servers, the TM would still be able to execute the best match for an order among all matches found by the matchmaking servers by simply aggregating the incoming matches for a specific order during some time.
If there is at least one honest matchmaker that has established the current best match for a specific order, the TM will execute this match, therefore nullifying the impact of the manipulation by the market operator.
However, this replicated matchmaking architecture raises the following performance and correctness concern:

\emph{1) Is it required to disseminate a new order to all matchmaker servers?}
In the replicated matchmaking model, we assume that a user sends a new order to all matchmaking servers.
...
The problem is that a flooding-based message dissemination strategy leads to a serious performance degradation when the number of matchmaking servers increases, as illustrated by deployed peer-to-peer applications like Gnutella~\cite{chawathe2003making}.
We address this concern by sending a new order to a small, random subset of all matchmaking servers (further described in Section~\ref{sec:order_dissemination}).

\emph{2) How does a replicated matchmaking system process matches of the same order found by distinct matchmaking servers?}
...
%Our replicated architecture can result in the scenario where distinct matchmakers submit different (valid) matches for the same order.
Assuming a FCFS execution of incoming matches by the TM, having multiple matchmaking servers sending matches to the TM can result in the situation where matches of the same order are executed multiple times, resulting in an incorrect order state.
To ensure correctness, our replicated matchmaking architecture requires logic to avoid that the same match is executed multiple times.
When performing matchmaking on a blockchain, this issue is addressed by reaching consensus on a serialized sequence of the matches that are to be executed.
Unfortunately, reaching this consensus is a resource-intensive process and severely limits the efficiency of blockchain technology for matchmaking, as we will experimentally show in Section~\ref{sec:experiments_ethereum}~\cite{vukolic2015quest}.
Our answer to this correctness concern is to have matchmakers only \emph{notify} users about potential matches.
...
Upon reaching a trade agreement, both negotiating parties forward this agreement to the TM.
...
\begin{figure*}[t]
	\centering
	\includegraphics[width=\linewidth]{match/assets/matching_protocol}
...
We now elaborate the system and threat model of MATCH.
...
We adopt a continuous-auction model where incoming orders are matched in an opportunistic, first-come-first-serve (FCFS) manner.
To model a two-sided market with supply and demand, we introduce two order types: \emph{offers} and \emph{requests}.
Offers indicate interest to sell specific assets, services, or resources whereas requests signal an interest in buying these assets, services or resources.
%Besides a few required fields (see Section~\ref{sec:order_creation}), the order specification is not fixed.
To ensure re-usability across different domains, matchmakers in MATCH can host multiple order books and therefore manage orders with differing specifications.
Each order has a quantity $ q $, which is an integer value indicating the amount of assets, services or resources being offered or requested.
The state of an order can be either \emph{open} (when the order has a positive quantity, $ q > 0 $), \emph{completed} (when all quantity in the order has been traded, $ q = 0 $), \emph{canceled} (when the order has been canceled by its creator) or \emph{expired} (when the timeout of the order has expired).
The structure and content of an order is elaborated in Section~\ref{sec:order_creation}.
...
\textbf{Actors.}
Peers in the MATCH network can act as \emph{user}, \emph{matchmaker}, or both.
Users submit offers and requests to peers that act as matchmaker.
MATCH requires active participation of users in order to negotiate with other users (also see Section~\ref{sec:order_negotiation}) and thus requires users to be online for their order to be completed.
...
We consider an unstructured peer-to-peer network where each peer (user or matchmaker) knows the network addresses of all matchmakers.
This can be achieved by maintaining a list of matchmakers, e.g., on a website or through a decentralized publishing network like the Kademlia DHT.
New matchmakers enroll themselves on this list while matchmakers leaving the network un-enroll from this list.
As we will show in Section~\ref{sec:order_dissemination}, MATCH is able to deal with offline  matchmakers that are still enrolled on the list.
...
Each peer possesses a public and private key, used to identify the peer and to sign outgoing network messages, respectively.
Thus, all messages between peers are authenticated.
We assume that the digital identity of each peer in MATCH is tied to a real-world identity, preventing uncontrolled identity creation (potentially resulting in a Sybil attack).
...
\subsection{Threats to Fairness}
\label{sec:threat_model}
In this work, our threat model orients around \emph{malicious matchmakers} that pursue selfish interest and match incoming orders according to their own policy.
For example, a matchmaker can deliberately match an incoming offer with the second-best request in their order book and match one of their own offers with the hidden request instead, all without being exposed.
The motivation to deviate from an agreed matching policy is often driven by economic incentives, e.g., when a matchmaker wants to match its own orders first.
Malicious matchmakers affect market fairness since incoming orders could now be treated unequally.
Matchmakers can also collude with other malicious matchmakers in order to gain economic benefit as a group.
We assume that cryptographic protocols are secure and that malicious matchmakers are unable to forge digital signatures.
...
%This also means that any broker can decide to ignore incoming orders and not process then further, essentially censoring the orders created by specific peers.
...
In summary, the protocol proceeds as follows.
...
Some period after receiving the first match for a specific order, a user selects an item stored in the associated match queue (\textcircled{4}), starting with the best match, and negotiates with the user behind the matched order (\textcircled{5} - \textcircled{7}).
When the negotiating parties reach a trade agreement (in other words, intend to fulfill their orders with each other), they execute the negotiated trade agreement by sending it to the TM (\textcircled{8} + \textcircled{9}).
These parties then inform the matchmakers about the trade so they can update the state of the affected orders accordingly (\textcircled{10} + \textcircled{11}).
The matchmakers are also informed on a negative negotiation outcome.
...
The MATCH protocol is now explained in more detail.

\begin{lstlisting}[language=json,firstnumber=1,float=t,caption=Example of an order in a ride-hailing market (in JSON format).,label=lst:order_example]
{
...
The content of the \texttt{data} field is flexible and depends on the trading context in which the order is created.
...
The order type is used by matchmakers to apply the correct policies for validation and matching, and to store the order in the appropriate order book.
...
After order creation, a user serializes it in an \texttt{order} message and sends it to matchmakers.
...
A \texttt{cancel} message for order $ O $ should be sent to the same matchmakers as the \texttt{order} message that contained the description of $ O $.
To achieve this, users keep track of the matchmakers to which they have sent an \texttt{order} message.
Upon reception of a \texttt{cancel} message, matchmakers remove the canceled order from their order book.
...
\begin{figure}[b]
	\centering
	\includegraphics[width=.75\linewidth]{match/assets/matching_idea}
...
$ R_{req} $ and $ R_{off} $ indicate the sets of matchmakers that receive a specific request and offer, respectively.
The probability that at least one matchmaker will receive both a specific offer and request quickly goes to one, even when the order fanout is low compared to the number of matchmakers (this is due to the birthday paradox).
...
Even if $ m = 100'000 $ and $ f = 500 $ (i.e., orders are sent to only 0.5\% of all matchmakers), the probability that at least one matchmaker receives both a compatible offer and a request, is already 97.7\%.
In this setting, it reduces the required fanout of an \texttt{order} message from 200'000 (when disseminating a new order to all matchmakers) to merely 1'000, therefore significantly reducing the network traffic required for order dissemination.
%Figure \ref{fig:matching_idea} illustrates the intuition behind this idea.
...
Given a target value for $ P(R_{req} \cap R_{off} \neq \emptyset) $, users compute an appropriate value of the order fanout themselves.
...
Equation~\ref{eq:dissemination} assumes that all matchmakers in the set $ R_{req} \cap R_{off} $ act in a fair manner and actually inform order creators on the best matches, given the orders in their order book.
Under our threat model, this is an unrealistic assumption for MATCH since matchmakers with selfish interests are actively undermining fairness.
Therefore, we account for the situation where a fraction $ r $ of all matchmakers pursue selfish interests.
Intuitively, this situation would require a higher value of $ f $ in order to reach at least one honest matchmaker.
We adjust Equation~\ref{eq:dissemination} and treat the response of a malicious matchmaker as if no response has been sent at all.
...
Equation~\ref{eq:malicious_dissemination} also accounts for matchmakers who are not informing users about possible matches for their order, either intentionally (e.g., when hiding matches) or unintentionally (e.g., when the matchmaker has crashed).
We show in the next subsection that this is an appropriate modeling of malicious matchmakers since their matches will likely be superseded by those from honest matchmakers when a user selects matches from the match queue.
...
Upon receiving a \texttt{match} message from a matchmaker, a user contacts the creator of the matched order and initiates a negotiation process (see Section~\ref{sec:order_negotiation}).
...
Each entry in the match queue of offer (or request) $ O $ is a tuple $ (a, R) $ where $ R $ is a request (or offer) that matches with offer (or request) $ O $, and $ a $ indicates the number of failed negotiation attempts for $ R $.
...
Before a user selects an item from a match queue, it waits for some duration $ W_{match} $, which we call the \emph{match window}.
...
$ W_{match} $ depends on the link latency of the network in which MATCH is deployed.
Furthermore, $ W_{match} $ is influenced by the trading domain, e.g., passengers in a ride-hailing market can usually tolerate an additional wait time of a few seconds, whereas this increase might be unacceptable when a user quickly wants to buy some assets in response to events in an asset market.
...
This negotiation approach increases resilience against malicious matchmakers since rational users choose to negotiate about the best incoming matches.
When two negotiating users reach a trade agreement, both users send the agreement to the trading mechanism, upon which the trade is executed.
This approach addresses question 2 in Section~\ref{sec:towards_fair_reliable_matchmaking} and requires minimal changes to the logic of the trading mechanism.
...
Order negotiation, based on entry $ (a, O) $ in the match queue associated with $ R $, starts by $ U_2 $ locking quantity in $ R $.
How much quantity is locked depends on the quantities in both $ O $ and $ R $.
Specifically, a user ($ U_2 $ in our example) proposes to trade as much available quantity as possible, given the specifications of $ O $ and $ R $.
Locking quantity in an order prevents a user from engaging in parallel negotiations for the same resources.
MATCH does not enforce this lock since rational users will correctly manage their order quantities.
Next, $ U_2 $ sends a \texttt{propose} message to $ U_1 $, containing the full specifications of $ R $, the identifier of $ O $, and the proposed quantity to trade.
...
We elaborate this control flow in more detail, using the negotiation between users $ U_1 $ and $ U_2 $ as example.
When $ U_1 $ receives a \texttt{propose} message from $ U_2 $, it first checks if its order contained in the \texttt{propose} message, $ O $, is still open.
If $ O $ is expired, has been canceled, or has been completed already, $ U_2 $ responds with a \texttt{reject} message, containing the reject reason.
When $ U_2 $ receives this \texttt{reject} message, it unlocks the locked quantity for that negotiation and selects the next item from the match queue of its request $ R $.
Since matchmakers might have outdated information about $ O $ (e.g., when $ O $ has been completed), $ U_2 $ forwards the \texttt{reject} message to the matchmakers that informed $ U_2 $ about the match with $ O $.
Matchmakers then update the state of $ O $ accordingly when receiving a \texttt{reject} message.

If offer $ O $ is open, $ U_1 $ determines if it has any unlocked quantity in the order left.
If there is no unlocked quantity available for trade, $ U_1 $ sends a \texttt{busy} message back to $ U_2 $, indicating that it currently has no room for negotiation due to pending negotiations for that order with other users.
...
To prevent a user from immediately retrying to negotiate again, a user waits a random period (between 1 and 2 seconds) before sending out another \texttt{propose} message for an entry in the match queue with $ a > 0 $.
...
If the proposed quantity is unavailable in $ O $, $ U_1 $ makes a counter-proposal by locking as much quantity as possible in $ O $ and by sending a \texttt{proposal} message back to $ U_2 $ with a different (but lower) quantity.
$ U_1 $ and $ U_2 $ keep sending \texttt{propose} messages with differing quantities until one of them responds with an \texttt{accept} or a \texttt{reject} message.
...
Therefore, all outgoing messages by a during order negotiation have a fixed \emph{negotiation window}, indicated by $ W_{neg} $, after which the user leaves the negotiation.
When this window expires, any locked quantity for this negotiation is released so it can be re-used in other negotiations for the same order.
...
The TM executes the trade after having received both messages.
...
In the following, we elaborate on the components in the MATCH middleware architecture.
...
This layer can be implemented using any networking library with support for peer-to-peer communication.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=.75\linewidth]{match/assets/matching_architecture}
...
It inserts incoming \texttt{match} messages to the appropriate match queue, discussed in Section~\ref{sec:match_queue}.
...
These validation policies can, for example, be distributed through a website and downloaded by users and matchmakers that are interesting in participation within a specific trading domain.
Incoming orders deemed invalid by the validation policy are discarded and not processed further.
...
\textbf{Order books.}
Each matchmaker can host multiple order books, colored green in Figure~\ref{fig:matching_architecture}, which store orders with differing types.
For example, MATCH can maintain a stock trading order book that stores orders to buy or sell Euros for Dollars, and another ride-hailing order book that stores transportation requests and ride offers.
Maintaining multiple order books is a key property of MATCH and results in a single and reusable matchmaking solution that can be deployed within different domains.
New order books are created on demand, based on the type of incoming orders.

\textbf{Matching engine.}
Valid incoming orders are passed to the matching engine, colored red in Figure~\ref{fig:matching_architecture}.
...
This state is stored in a \emph{negotiation store}, colored yellow in Figure~\ref{fig:matching_architecture}.
...
This guarantee is, for example, provided by smart contracts, applications that runs on top of a blockchain (also see Section~\ref{sec:experiments_ethereum})~\cite{Luu2016MakingSC}.
...
The network layer is implemented using our networking library, optimized for building peer-to-peer overlay networks and with built-in support for Network Address Translation (NAT) puncturing and authenticated messaging.\footnote{Library name omitted for double-blind review.}
For efficiency, message exchange between users and matchmakers uses UDP protocol.
Order negotiation proceeds using the TCP protocol since this flow requires bilateral and reliable message exchange.
All software artifacts of MATCH (source code, tests, and documentation) are available online.\footnote{Can be requested through the Program Chair.}
...
In Section~\ref{sec:experiments_ethereum}, we compare our solution to matchmaking on an Ethereum blockchain and show that MATCH uses considerably less bandwidth and memory, and has superior order fulfill latencies.
...
All experiments are conducted on our nation-wide university cluster, allowing us to run multiple instances of MATCH on different compute nodes.
...
These values should be increased when MATCH is deployed in networks with higher link latency, since it then might take longer for \texttt{match} and negotiation messages to be received.
...
The first set of experiments focuses on the matching quality and fairness of the MATCH middleware under a ride-hailing workload.
This workload contains temporal information about ride offers and requests created by drivers and passengers, respectively.
Each order in the ride-hailing workload has a quantity of one, ensuring that a ride request is matched with at most one offer (one-to-one matching).
...
We analyze the traces and subtract 2'100 ride offers and requests during the busiest period in 2015: November 1, 00:59:57 to November 1, 01:01:16 (datasets published after 2015 did not include geographic information on drivers and passengers).
...
For the ride-hailing workload, we implement the matching policy such that it minimizes the distance between passengers and drivers.
Specifically, the policy computes the geographic (haversine) distance between the location included in offers and requests.
In this context, matching quality is defined as the average distance between matched passengers and drivers, which we also call the \emph{match distance}.
This quality metric follows related work on ride-hailing~\cite{pham2017oride}.
...
The matching quality decreases significantly when $ f = 30 $ and $ m $ increases. E.g., for $ m = 2'000 $, the average match distance increases to 0.607km, 9.5\% higher compared to the baseline.
...
The overall match quality, however, might be improved when batching incoming orders, since it could have been better to assign an already-matched driver to a passenger that created its request later during the experiment.
...
In MATCH, users aggregate potential matches during the match window, $ W_{match} $, resembling this behavior.
Therefore, the matching quality in MATCH can exceed that of centralized FIFO matchmaking because of match aggregation by users, at the cost of a larger order fulfill time.

\textbf{Impact of selfish matching.}
We show how selfish behavior of malicious matchmakers impacts the quality in MATCH.
We model a malicious matchmaker as a driver that matches an incoming ride request from a passenger with its own service offer.
This captures the economic incentive of drivers to prioritize their own services during matchmaking.
...
We vary $ r $, up to 50\% of all matchmakers ($ r = 0.5 $).
...
Therefore, we also consider a adaptive order fanout, based on the values of both $ r $ and $ m $.
...
In practice, the exact value of $ r $ is not known a-priori and MATCH developers should therefore fix the order fanout to account for an upper bound for the malicious fraction (for example, many BFT consensus algorithms tolerate up to $ r = \frac{1}{3} $).
...
The ride-hailing workload involves offers and requests that are matched on a one-to-one basis and are never canceled.
When deploying MATCH in a domain where digital assets are traded, orders can often be partially fulfilled and are commonly canceled (e.g., in high-frequency trading).
Therefore, we conduct the same experiments described in Section~\ref{sec:taxi_experiments} with an asset trading workload. % the performance of MATCH is evaluated with an asset trading workload.
Again, the focus is on the match quality and the impact of malicious matchmakers.

To the best of our knowledge, there is no standardized definition for the matching quality of orders with partial fulfillment.
Therefore, after each experiment with the asset trading workload, we determine the matching quality as follows: all orders that are not canceled or fulfilled are added to a single order book, starting with the first order created.
...
\textbf{Workload specification.}
The asset trading workload contains buy and sell orders that have been published on the blockchain ledger of BitShares~\cite{schuh2015bitshares}.
BitShares is a blockchain-based decentralized exchange where users can create, issue and trade digital assets.
New orders are submitted to dedicated validator peers, which include incoming orders in a block on the blockchain.
We have analyzed the entire BitShares blockchain since its inception and extracted all buy and sell orders, and order cancellations.
...
Under the asset trading workload, we model a malicious matchmaker as a peer that purposefully does not inform the party behind an incoming order about the match with the best price.
...
We compare the bandwidth usage, memory usage and order fulfill latencies of MATCH with that of matchmaking on an Ethereum blockchain, using both the ride-hailing and asset trading workloads.
%Specifially, we conduct matchmaking on a private Ethereum blockchain.
Ethereum is the most mature blockchain platform that enables the execution of generic-purpose smart contracts, and is the most used platform to deploy smart contracts in general.

We setup a private Ethereum network consisting of 400 instances running \texttt{geth}, an Ethereum client written in Go.\footnote{https://github.com/ethereum/go-ethereum}
Ethereum uses a Proof-of-Work consensus mechanism in which participants, also called miners, compete with their compute resources.
Specifically, each miner continuously solves an algorithmic puzzle and the first miner to find a valid solution to the puzzle, can extend the blockchain with one block.
...
For each workload, we implement a smart contract in the Solidity programming language.
The smart contract for the ride-hailing workload maintains two lists containing open offers and requests.
Submission of a new ride offer and request is done by issuing a transaction with geographic information that invokes the \texttt{ride\_offer} and \texttt{ride\_request} method, respectively.
...
Compared to using latitude and longitude coordinates in combination with the Haversine distance, this conversion results in an accuracy loss of only 0.35\%.

For the asset trading workload, we adopt an existing and deployed order book implementation.\footnote{https://github.com/makerdao/maker-otc/tree/master/src}
This smart contract implements a market to trade digital tokens that reside on the Ethereum blockchain.
To quantify the overhead of order matchmaking, we remove the token transfer after matching from the smart contract but leave the implemented price-time matchmaking logic intact.
...
\begin{figure*}[t!]
	...
	\begin{subfigure}{.33\columnwidth}
		\centering
		\includegraphics[width=\columnwidth]{match/assets/plots/ethereum_memory.pdf}
	...
	\begin{subfigure}{.33\columnwidth}
		...
		\caption{Order fulfill latency}
		\label{fig:ethereum_latencies}
	\end{subfigure}
	\caption{The total bandwidth usage, the maximum memory usage across matchmakers, and the distribution of order fulfill latencies of on-chain matchmaking on Ethereum and in MATCH, under the ride-hailing and asset trading workloads.}
...
The high bandwidth usage of Ethereum is a direct consequence of the requirement for full state replication.
Specifically, each submitted transaction and produced block is disseminated to all active Ethereum instances, resulting in a significant amount of network traffic.
...
Specifically, this is the time between order creation and order fulfillment.
...
Since users aggregate \texttt{match} messages during the match window, the order fulfill latency in MATCH is at least $ W_{match} $ (which is fixed to two seconds in our experiments).
In comparison, the average order fulfill latency in Ethereum is 258.2 and 53.6 seconds under the ride-hailing and asset trading workload, respectively.
...
\begin{figure}[b]
	\centering
	\includegraphics[width=.75\linewidth]{match/assets/plots/tx_pool}
	\caption{The size of the Ethereum transaction pool when running the ride-hailing and asset trading workload.}
	\label{fig:ethereum_tx_pool}
\end{figure}

\textbf{Ethereum transaction pool.}
To further analyze the large differences in order fulfill latencies between MATCH and Ethereum, we visualize the size of the Ethereum transaction pool (as maintained by a single Ethereum instance) during the experiment in Figure~\ref{fig:ethereum_tx_pool}.
...
We explain this behavior as follows.
In Ethereum, the sum of gas usage of all transactions in each block cannot exceed 10'000'000 gas.
The gas cost of matching ride requests scales with the number of open ride offers and decreases over time during the experiment since there are less offers to compare with.
Note how after 500 seconds into the experiment, the number of unconfirmed transactions decreases more quickly for the ride-hailing experiment.

%Overall, our comparison experiments highlight the resource costs associated with transparent matchmaking.

\section{Related Work}
Matchmaking (or brokering) is a core concept in publish/subscribe architectures.
In centralized publish/subscribe architectures, a single server brokers incoming messages between publishers and subscribers~\cite{Carzaniga2001DesignAE}.
Decentralized approaches either flood events through the entire network, or route these events based on their topic or content~\cite{Banavar1999AnEM}~\cite{Carzaniga2004ARS}.
...
Resource allocation, the assignment of compute resources to incoming jobs, requires matchmaking~\cite{czajkowski1998resource}.
...
The first approach is to use market mechanisms that coordinate the matchmaking process, e.g., by a continuous double auction mechanism~\cite{gomoluch2003market} \cite{mihailescu2010distributed} \cite{Buyya2002EconomicMF}.
...
The second matchmaking approach is to deploy one or more dedicated (centralized) brokers~\cite{raman1998matchmaking} \cite{frey2002condor} \cite{ebrahimi2004matchmaking}.

Motivated by the scalability and load balancing issues of centralized matchmaking, various researchers explored the usage of multiple, independent matchmakers~\cite{azab2008adaptive}~\cite{edinger2016decentralized}~\cite{Abdullah2010EffectOT}~\cite{sigdel2005framework}.
Matchmakers usually operate within their own administrative domain, acting as a broker for a specific set of peers.
...
The work of Shafran et al. evaluates a distributed matchmaking model where orders are cached by intermediate agents~\cite{Shafran2008TowardsBD}.
...
In AirSwap, \emph{indexers} mediate trade between makers, peers who create an order, and takers, who fulfill existing orders~\cite{swap}. %, and act as matchmakers.
...
Their protocol description, however, lacks details on the dissemination strategy of orders to matchmakers.
...
\section{Conclusions}
We have presented MATCH, a decentralized middleware for fair order matchmaking.
...
In the MATCH protocol, users send new orders to a small, random subset of matchmakers, which then inform users about potential matches.
Users then engage in peer-to-peer negotiation about matches with other users.
This approach makes MATCH resilient against matchmaker with selfish interests.
...
We have experimentally proven resistance against malicious matchmakers in a ride-hailing and asset trading domain, showing that MATCH still establishes matches with high quality.
