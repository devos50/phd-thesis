\chapter{DevID: Decentralized Developer Portfolios for Software Crowdsourcing}
\label{chapter:devid}
...
The main problem is that centralized approaches for client-developer matching lead to \emph{fragmentation} and \emph{lock-in} effects~\cite{pouwelse2017laws}.
...
DevID distinguishes between the following six record types:
...
\todo{expand on the transaction validation process}
...
We now elaborate on the main functionalities of dAppCoder.\todo{why competition?}
...
\todo{what about private projects?}
...
Each project can be in one of the five following phases: \emph{created}, \emph{implementing}, \emph{testing}, \emph{finished} or \emph{cancelled}.
...
When the deliverable is satisfactory, the client indicates that the project is finished and instructs the trusted notary service to payout involved developers.\todo{elaborate on payouts}
