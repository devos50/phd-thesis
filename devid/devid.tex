...
At the same time, recruiting developers with appropriate qualifications and skills for this activity is key, yet challenging.
...
\emph{In this chapter we address these problems and first introduce a blockchain-based portfolio for developers, named \emph{DevID}.
...
All portfolio records are stored on a scalable distributed ledger, named TrustChain, and owned by developers themselves.
TrustChain enables tamper-proof data accounting with low overhead and without network-wide consensus.}

\emph{We then build a crowdsourcing platform, named \emph{dAppCoder}, for the development of decentralized applications.
On dAppCoder clients are able to submit their ideas and developers can find work, all without trusted intermediaries.
...
We believe that such a portfolio is a key precursor to a more efficient crowdsourcing marketplace, since highlighting the expertise of a developer is a key activity.
...
We then build a crowdsourcing platform for the development of decentralized applications, named \emph{dAppCoder}.
Our platform enables clients to post a project and developers to find projects that match with their expertise.
All payments are conducted directly between clients and developers, sidestepping a trusted intermediary for payment processing and consequentially avoiding commissions.
...
We believe that our single, public, and open market has the potential to be more efficient compared to a centralized solution with fragmentation and lock-in effects.
...
The main challenge of this work is two-fold.
First, we wish to create a developer portfolio that gives an accurate impression of the capabilities and expertise.
Second, we aim to build a decentralized crowdsourcing platform for the development of dApps.
We elaborate on the challenges associated with each contribution.

\textbf{Developer portfolios.}
We identify two main requirements for this portfolio.
...
Second, we require our developer portfolio to be \emph{independent of any trusted intermediary}.
...
Instead, all information should be managed in a decentralized manner, and by the network itself.
...
We believe that the above record types are sufficient to enable developers to devise a basic portfolio.
The implementation of DevID is flexible and enables system designers to add additional record types later.
...
The user interface of DevID displays a special badge on the profile of verified users.
...
TrustChain enables users to issue transactions with an application-specific payload, and append these transactions to their personal ledger.
It also enables system designers to specify validity rules for different transaction types.
...
Users joining the network continuously request TrustChain transactions from other users and consequently build up knowledge on the DevID portfolios of others.
...
\label{sec:dappcoder}
By extending the DevID portfolio architecture, we build a crowdsourcing marketplace for the development of decentralized applications.
...
The process of finding developers is streamlined by DevID portfolios.
...
If sufficient developers are interest to work on a particular project, the client selects developers, for example, based on the information in their DevID portfolios.
This information is then published in a \texttt{dappcoder\_project\_update} transaction that transitions the project from the \emph{created} to \emph{implementing} stage.
During the implementation stage, assigned developers work on implementing the project deliverables.
When the implementation deadline passed, the project enters the \emph{testing} stage.
...
If the source code if the project can be published, a developer may create a \texttt{dappcoder\_submission} transaction that includes a pointer to the work done.
This pointer, for example, can refer to a GitHub repository.
The transaction also links to the project associated with the submission.
...
The time at which these payouts take place should be decided between the developer and client.
For example, a developer can demand to be partially compensated up-front.
...
Clients that have no valid payout associated with a finished, paid project will be flagged in the application and will be avoided by developers.
...
By building upon a scalable distributed ledger, DevID is capable of storing tamper-proof records and does not depend on any trusted party.
