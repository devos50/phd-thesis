...
The sharing economy is a prime example of these phenomena.
...
Each peer organizes its records in a \emph{personal ledger}.
...
These techniques, however, are not generic and cannot easily be used within many application domains.
...
After peer $ a $ has included all described attributes in the proposal, it persists the record to its database, sends the proposal to $ b $, and disseminates the proposal to $ f $ random peers in the network (step \circled{2}).
...
After the creation of $ C $, peer $ b $ persists the confirmation to its database, sends it to peer $ a $, and disseminates both $ P $ and $ C $ to $ f $ random peers (step \circled{4}).
...
\begin{enumerate}
	\item First, we include a sequence number $ s \in \mathbb{Z} $ in each record that is incremented by one when a record is added to one's personal ledger.
...
This approach would require fundamental changes to \TrustChain{}, and we therefore leave this enhancement for further work.
...
This modification would result in pairs of records with the same sequence number and the same creator, but with a different hash, and violates the integrity of the \TrustChain{} data structure.
...
\begin{itemize}
	...
	A peer that encounters this situation sends the pair ($ R_1 $, $ R_2 $) within an \texttt{Inconsistency} message to other random peers, hoping that others will be able to expose the malicious peer.
	...
	Similar to scenario III, the validating peer sends the pair ($ R_1 $, $ R_3 $) within an \texttt{Inconsistency} message to other, random peers.
\end{itemize}
...
This step is performed by the \textsc{validateLink} procedure which pseudocode description is given in Algorithm~\ref{alg:record_validation_step3}.
...
This layer can be realized using existing frameworks to build peer-to-peer overlay networks, for example, libp2p.\footnote{See \url{https://libp2p.io}}
...
Figure~\ref{fig:experiment_crawl_interval_detection_times} visualizes the impact of the record request interval on the average fraud detection times for different record exchange strategies.
...
A pseudocode description of the slot assignment logic is given in Algorithm~\ref{alg:slot_logic}.
...
We also observed two large-scale outages of exit nodes, in April 2019 and May 2020, likely due to infrastructure failures.
