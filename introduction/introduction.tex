...
In the context of this work, \emph{a blockchain-based marketplace is a distributed marketplace that leverages blockchain technology to carry out one or more of its critical operations}.
...
Electronic marketplaces require a mechanism for the management of market information.
Typical market information includes pricing and product data, and details of past transactions (which is often used to estimate the trustworthiness of market participants).
Traditional electronic marketplaces take a centralized approach and store this information on their servers, deployed and fully managed by the market operator.
A key advantage of this approach is that centralized servers are relatively easy to setup and maintain.
Furthermore, they enable fast access to market information by participants.
Since the market data is managed by the operator, it is prone to manipulation.
Market operators can also influence what information is being served when being requested.

We find that blockchain-based marketplaces tend to leverage decentralized techniques to store and share their market information.
We identify two common approaches for data storage and dissemination, which are explained next.

\subsubsection{Distributed Ledger}
A common approach for blockchain-based marketplaces is to persist all market information on a distributed ledger, e.g., a blockchain.
This is the approach taken by many DEXes where the full market state is bundled within tamper-proof transactions on a distributed ledger, secured by a consensus mechanism.
Users wishing to get the latest market state are required to request the full distributed ledger from the network.
Since blockchain is an append-only data structure, no information is ever removed from the distributed ledger.
Therefore, some blockchains have high storage requirements, e.g., the entire Bitcoin blockchain requires around 270GB of storage.
Some blockchain-based markets provide one or more \emph{full nodes} that remain synchronized in the network and can be queried by interested traders.

\subsubsection{ InterPlanetary File System (IPFS)}
The high costs associated with storing data on a blockchain has motivated some blockchain-based marketplaces to leverage another storage mechanism.
For example, it is not feasible to store large data elements on a blockchain ledger.
...
IPFS builds on top of libp2p~\cite{benet2018libp2p}, a networking framework built for decentralized protocols.
...
\emph{Matching} buyers and sellers is a prerequisite for online trade and therefore essential for marketplaces.
...
For instance, prolonged suboptimal matching by Uber increases the waiting time for passengers and results in drivers having to traverse a greater distance to pick up their customers.
...
Centralized matchmaking is the most common approach to match market orders.
Traders send new offers and requests to a dedicated matchmaker, usually a centralized system (server).
Incoming offers and requests are matched with other requests and offers, respectively, according to a \emph{matching policy}.
This model is widely adopted by commercialized marketplaces such as stock exchanges or service markets like Uber.
The EtherDelta~\cite{etherdelta} and IDEX~\cite{idex} decentralized exchanges adopt a centralized matchmaking model.

A main advantage is that the centralized matchmaking model with a single server is relatively straightforward to implement, since no communication between peers, or order book synchronization is required.
...
For example, with centralized matchmakers, market operators are able to censor or delay specific orders.
Information asymmetry between exchange operators and traders allows operators to exploit their information advantage, e.g., by front-running specific orders.
...
Another concern is fault tolerance: if the single matchmaker becomes unavailable, e.g., due to infrastructure failures, incoming orders cannot be matched and all market activity stalls.
...
We refer to this approach as \emph{federated matchmaking}.
...
The 0x~\cite{0x} and Swap protocols are notable examples of the federated matchmaking model.
These protocols enable any users to act as matchmaker and therefore build an off-chain matchmaking network for orders that are trading tokens on Ethereum.
...
Federated matchmaking gives traders the opportunity to select their preferred matchmaker.
When a matchmaker is suspected to treat incoming orders unfairly, traders can entrust their orders to another matchmaker instead.
Furthermore, the unavailability of an individual matchmaker is less likely to stall all market activity since a trader can send their orders to another available matchmaker.
However, the order book is fragmented across different matchmakers, potentially leading to less market efficiency and trading opportunities compared to centralized matchmaking.
...
The main idea of \emph{decentralized matchmaking} is that a single order is sent to multiple matchmakers simultaneously and matchmakers share their orders with other matchmakers.
We distinguish between on-chain and off-chain decentralized matchmaking.

\textbf{On-chain.}
On-chain decentralized matchmaking relies on a smart contract to match incoming orders, or execute the matchmaking logic as part of the transaction validation.
The orders are embedded in transactions and sent to multiple miners for inclusion on the blockchain.
For example, Stellar has integrated DEX functionality that allows users to issue buy and sell orders for assets native to the Stellar blockchain~\cite{lokhava2019fast}.
These orders are created using specific transaction types, that are matched with existing, outstanding orders already on the ledger.
Similarly, the BitShares DEX offers specialized transactions to create new orders~\cite{schuh2015bitshares}.
...
However, since users need to pay fees when creating new orders or when cancelling existing ones, order management can become costly.
...
On-chain matching protocols do not explicitly store established matches.
Therefore, to reconstruct the order book at a specific block height, one needs to process all transactions up to that point.

\textbf{Off-chain.}
To increase trading efficiency, some blockchain-based marketplaces maintain the order book off-chain.
Loopring is an order sharing protocol where new orders are sent to one or more relays in a mesh network~\cite{loopring}.
Relayers take the margin between two matched orders, or can charge a fixed fee for their services.
...
The protocol uses Shamir secret sharing to break down an order in multiple order fragments which are distributed through the network.
...
First, sharing orders between matchmakers can yield the same matching effectiveness compared to centralized matchmaking, depending on how orders are synchronized amongst matchmakers. %since orders can be synchronized amongst matchmakers.
...
In addition, it might take longer before a new order is fulfilled in the case that it is sent to matchmakers that are unable to match this order immediately.

\subsection{Settlement}
Blockchain-based marketplaces require a mechanism for a transaction to complete.
The process of fulfilling the obligations by trading partners is also called \emph{settlement}.
We now elaborate on four settlement techniques commonly found in blockchain-based marketplaces.
...
An exchange is considered fair if either both of the parties receive the items they expect, or none of them do.
The FairSwap protocol ensures fair exchange of digital goods by leveraging smart contracts~\cite{dziembowski2018fairswap}.
The protocol is designed around the exchange of digital commodities and is therefore not usable for generic asset exchange across different blockchains.
...
Optimistic fair exchange has originally been applied for the exchange of digital signatures but has recently been leveraged to ensure the execution of a cryptocurrency payment in exchange for a receipt~\cite{liu2018toward}.
...
Compared to an asset exchange through a trusted intermediary, notary schemes assume a weaker trust model and can often withstand adversarial behaviour of a fraction of the notaries.
...
Fraud management, however, becomes instrumental when the settlement process is decoupled and requires both involved traders to transfer value to their counterparty. % depends on the actions by users.
We identify two common approaches to manage fraud arising during a non-atomic trade: using escrow services and collateral.
...
In the Bisq decentralized exchange, for example, users can make a call on mediators or arbitrators to resolve fraud.
Mediators attempt to resolve the dispute but do not have authority over the funds being traded.
Arbitrators, however, have the capability to redistribute assets being traded through the usage of multisignature techniques.
...
The XClaim protocol relies on collateral deposits to enable asset trading between distinct blockchain ledgers~\cite{zamyatin2019xclaim}.
When a participant deviates from the protocol, the collateral is slashed and wronged actors are reimbursed.

\subsection{Identity Management}
Users join a blockchain-based marketplace under a digital identity.
Within the domain of blockchain-based marketplaces, we discuss two approaches to identity management: using pseudonyms and using real-world identities.
