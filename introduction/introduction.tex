...
\dropcap{A} marketplace facilitates the exchange of services, goods and information between individuals and businesses.
...
This thesis introduces five \emph{decentralized} solutions for blockchain-based marketplaces.
...
Despite much hype surrounding the Bitcoin ecosystem, we point out three limitations of the Bitcoin cryptocurrency.
...
This approach exclusively used in the context of blockchain-based marketplaces, to the best knowledge of the authors.
...
The protocol uses Shamir secret sharing~\cite{shamir1979share} to break down an order in multiple order fragments which are distributed through the network, therefore hiding the identity of the order creator.
...
Atomic swaps enable two parties to exchange blockchain-based assets atomically: the asset exchange either completes or fails for both parties at any given time.\footnote{We remark that the atomicity property of the atomic swap depends protocol on the security of the underlying blockchains. If one of the blockchains is compromised by adversaries, atomicity during asset exchange cannot be guaranteed and one of the parties can lose its funds to the counterparty.}
Atomic swaps eliminate the risk of losing assets to an adversarial trader during the exchange.
The main idea is that trading users lock assets in a specialized transaction on the blockchain and both acknowledge the receipt of assets within a specific time period.
This is achieved with \emph{Hash-Timelock Contracts}, a special transaction that leverages hash locks and time locks.
A hash lock is a restriction that prevents the transfer of funds until the pre-image of a provided hash is revealed.
A time lock is a primitive that locks funds until a specific time and prevents funds from being locked up for an indefinite amount of time during an atomic swap.
This time lock should be well above the block confirmation time of the underlying blockchain to prevent the loss of funds during a blockchain reorganization.
In practice, this value is often fixed to 24 hours.

We further explain the atomic swap by considering an asset exchange between Alice and Bob.
Figure~\ref{fig:atomic_swap_intro} visualizes an atomic swap between Alice and Bob, where Alice sells her Bitcoin in return for Ether (the native token of the Ethereum blockchain).
The basic atomic swap, first described by Tier Nolan, consists of the following six steps:

\textbf{Step 1.} Alice generates a secret value $ s $ and computes $ H(s) $ where $ H(\cdot) $ is a secure hash function.

\textbf{Step 2.} Alice submits a hash-timelock transaction $ T_1 $ to the Bitcoin blockchain, locking her Bitcoin and using $ H(s) $ for the hash lock. A party can claim these Bitcoin with another transaction that provides $ s $ within a specific time duration.

\textbf{Step 3.} Alice sends the value of $ H(s) $ to Bob.

\textbf{Step 4.} Bob submits a hash-timelock transaction $ T_2 $ to the Ethereum blockchain, locking his Bitcoin and using $ H(s) $ for the hash lock.

\textbf{Step 5.} Since Alice knows $ s $, she claims the Bobs' Ether locked in $ T_2 $ by submitting a transaction, $ T_3 $, to the Ethereum blockchain, containing $ s $. This reveals pre-image $ s $ to Bob.
...
\textbf{[RQ2] How can market orders be matched without a centralized coordinator?}
...
\textbf{[RQ3] How can the settlement durations of (international) payments be improved?}
...
\textbf{[RQ4] How can assets securely be exchanged between any blockchain without a trusted intermediary?}
...
\textbf{[RQ5] How can blockchain technology be applied to disintermediate in software crowdsourcing markets?}
...
\begin{table*}[t]
	...
	\begin{tabular}{ |c|c|c|c| }
		\hline
		\textbf{RQ} & \textbf{Mechanism} & \textbf{DAS5 experiments?} & \textbf{Deployment} \\ \hline
		1 & TrustChain & yes & integration in Tribler \\ \hline
		2 & MATCH & yes & integration in Tribler \\ \hline
		3 & XChange & yes & integration in Tribler \\ \hline
		4 & Internet-of-Money & yes & in-house testing \\ \hline
		5 & DevID & no & user trial \\ \hline
	\end{tabular}
...
Blockchain is a relatively new technology of which research is conducted in economics, computer science and social sciences.
We identify two main research approaches in this field.
On the one hand, published blockchain research in systems-oriented conferences (e.g., Middleware, DEBS and OSDI) adopt experimental methods where the performance of proposed mechanisms is evaluated using a comprehensive set of experiments and benchmarks.
...
This body of research tends to follow a theoretical approach through the use of formal proofs or protocol simulations.

In this thesis, we adopt an experimental approach where we answer each research question by designing, implementing and evaluating a decentralized system or mechanism.
...
For all of our proposed mechanisms, our goal is to build a solution that is ready for deployment and usable by real users.
We have implemented each mechanism in the Python programming language, often within a few thousand lines of code.
We utilize an existing networking library for networking primitives, which is developed by our lab.\footnote{https://github.com/tribler/py-ipv8}
Table~\ref{table:research_methodology} specifies for each mechanism whether we present a set of experiments on our nation-wide compute cluster (the DAS5~\cite{bal2016medium}) and how we have conducted our deployment.
Except for DevID, we evaluate each system with an appropriate set of experiments that have been conducted on the DAS5.
During this thesis, we worked with colleagues that do not have a background in distributed systems.
For example, Georgy Ishmaev framed the research problem that MATCH addresses within the broader scope of socio-ethical implication of matching platforms for peer-to-peer markets.

Driven by our focus on practicality, we have deployed each mechanism proposed in this thesis, either to a small group of colleagues or with an integration in Tribler~\cite{zeilemaker2011tribler}.
...
This longitudinal deployment has allowed us to incrementally improve our software and refine our ideas.
...
Due to legal considerations, we are unable to provide an open-source implementation of the four reverse-engineered banking algorithms in our Internet-of-Money mechanism.

The complexity of real-world systems often posed a challenge when deploying our mechanisms.
For example, shortly after the initial deployment of the MATCH mechanism in Tribler, a group of students forged a specific packet that would crash a Tribler instance.
Specifically, we forgot to check whether the timeout value of an order was within a valid range.
Tribler would forward the packet to other connected peers before crashing.
Therefore, this bug affected a large part of our network and we immediately deployed a fix for this behaviour by releasing a new version of Tribler.
Despite the large efforts required to get a system deployed and operational, this process has been tremendously helpful in discovering both critical design flaws and smaller implementation errors.
...
\textbf{[Chapter 3] MATCH: a Decentralised Middleware for Fair Matchmaking in Peer-to-peer Markets.}
